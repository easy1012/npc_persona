...
for ($attempt = 1; $attempt -le $MaxAttempts; $attempt++) {
    docker compose up -d api streamlit proxy
    $deadline = (Get-Date).AddSeconds($TimeoutSeconds)
    while ((Get-Date) -lt $deadline) {
        if (Test-Healthy) {
            $receipt = Save-Diagnostics $attempt
            Write-Host "Non-GPU services healthy. Log: $receipt"
            exit 0
        }
    ...
    docker compose stop api streamlit proxy
}

throw "Non-GPU services failed after $MaxAttempts attempts. Review logs in $LogDirectory."
